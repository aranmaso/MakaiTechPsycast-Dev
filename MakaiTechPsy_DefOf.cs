using RimWorld;
using Verse;

namespace MakaiTechPsycast
{
    [DefOf]
    internal static class MakaiTechPsy_DefOf
    {
        public static HediffDef MakaiPsy_LinkingBond;
        public static HediffDef MakaiPsy_BI_BondingPsychic;
        public static HediffDef MakaiPsy_MechHackOverload;
        public static HediffDef MakaiPsy_PK_Brain_Mulfunction;
        public static HediffDef MakaiPsy_BI_BondingCleanse;

        public static HediffDef MakaiPsy_BI_Trading;
        public static HediffDef MakaiPsy_BI_TradingFail;
        public static HediffDef MakaiPsy_BI_TradingGreat;

        public static HediffDef MakaiPsy_CP_TaintLevel;
        public static HediffDef MakaiPsy_SF_Reverse;
        public static HediffDef MakaiPsy_SF_Counter;

        public static HediffDef MakaiTechPsy_DD_LichSoul;
        public static HediffDef MakaiTechPsy_DD_CollectSoul;
        public static HediffDef MakaiTechPsy_DD_MissingSoul;
        public static HediffDef MakaiTechPsy_DD_Helheim;
        public static HediffDef MakaiTechPsy_DD_LifeLink;

        public static DamageDef TrueDestruction_LightningTower;
        public static DamageDef TrueDestruction_LightningTowerBeam;
        public static DamageDef CorruptedProphet_CorruptedSky;
        public static DamageDef DestinedDeath_Rot;
        public static DamageDef DestinedDeath_SharedDamage;
        public static DamageDef DestinedDeath_LinkExplosion;


        public static ThingDef MakaiTechPsy_DD_Soul;
        public static ThingDef MakaiPsy_Pull;
        public static ThingDef MakaiPsy_PullSlow;

        public static ThingDef MakaiTechPsy_PK_BeamSingle;
        public static ThingDef MakaiTechPsy_ReflectWarp;
        public static ThingDef MakaiPsy_DD_Bolt;
        public static ThingDef MakaiPsy_TD_Beam;
        public static ThingDef MakaiPsy_CP_Beam;
        public static ThingDef MakaiPsy_CP_Pillar;

        public static FleckDef MakaiPsyMote_DeathBolt_Trail;

        public static ThingDef MakaiTechPsy_CP_CorruptedTower;

        public static ThingDef MakaiPsyMote_YellowGlow;
        public static ThingDef MakaiPsyMote_PurpleRingSuck;

        public static EffecterDef MakaiPsy_DD_Blast;
        public static EffecterDef MakaiPsy_TD_Blast;
        public static EffecterDef MakaiPsy_Ring_ExpandY;
        public static EffecterDef MakaiPsy_WarpBullet;
        public static EffecterDef MakaiPsy_DD_Suck;

        public static BodyPartGroupDef Hands;


    }
}
