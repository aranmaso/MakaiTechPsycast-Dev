        ...
        public override void Cast(params GlobalTargetInfo[] targets)
        {
            ...
            if (rollinfo.roll >= modExtension.successThreshold && rollinfo.roll < modExtension.greatSuccessThreshold)
            {
                if (targets[0].Thing is Pawn targetPawn && targetPawn.health.hediffSet.HasHediff(MakaiTechPsy_DefOf.MakaiTechPsy_DD_LifeLink))
                {
                    ...
                    GenExplosion.DoExplosion(targetPawn.Position,pawn.Map,Mathf.Max(5,accumulateDamge/10),modExtension.damageDef,pawn,Mathf.FloorToInt(accumulateDamge),1f,null,null,null,targetPawn,null,0,0,GasType.RotStink);
            ...
            if (rollinfo.roll >= modExtension.greatSuccessThreshold)
            {
                if (targets[0].Thing is Pawn targetPawn && targetPawn.health.hediffSet.HasHediff(MakaiTechPsy_DefOf.MakaiTechPsy_DD_LifeLink))
                {
                    Hediff hediff = targetPawn.health.hediffSet.GetFirstHediffOfDef(MakaiTechPsy_DefOf.MakaiTechPsy_DD_LifeLink);
                    float accumulateDamge = hediff.TryGetComp<HediffComp_LifeLink>().totalDamage;
                    GenExplosion.DoExplosion(targetPawn.Position, pawn.Map, Mathf.Max(10, accumulateDamge / 5), modExtension.damageDef, pawn, Mathf.FloorToInt(accumulateDamge*1.25f), 1f, null, null, null, targetPawn, null, 0, 0, GasType.RotStink);
            ...
            if (rollinfo.roll < modExtension.successThreshold)
            {
                if (targets[0].Thing is Pawn targetPawn && targetPawn.health.hediffSet.HasHediff(MakaiTechPsy_DefOf.MakaiTechPsy_DD_LifeLink))
                {
                    Hediff hediff = targetPawn.health.hediffSet.GetFirstHediffOfDef(MakaiTechPsy_DefOf.MakaiTechPsy_DD_LifeLink);
                    float accumulateDamge = hediff.TryGetComp<HediffComp_LifeLink>().totalDamage;
                    GenExplosion.DoExplosion(targetPawn.Position, pawn.Map, Mathf.Max(3, accumulateDamge / 10), modExtension.damageDef, pawn, Mathf.FloorToInt(accumulateDamge*0.75f), 1f, null, null, null, targetPawn, null, 0, 0, GasType.RotStink);
