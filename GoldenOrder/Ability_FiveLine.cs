...
{
    public class Ability_FiveLine : VFECore.Abilities.Ability
    {
        ...
        public override void Cast(params GlobalTargetInfo[] targets)
        {
            ...
            if (rollinfo.roll >= modExtension.successThreshold && rollinfo.roll < modExtension.greatSuccessThreshold)
            {
                ...
                for(int i = 5;i > 0;i--)
                {
                    IntVec3 randomOrigin = GenRadial.RadialCellsAround(targets[0].Cell,GetRadiusForPawn(),true).Where(x => x.DistanceToSquared(targets[0].Cell) > (GetRadiusForPawn() - 1) * (GetRadiusForPawn() - 1)).RandomElement();
                    Projectile projectile = (Projectile)GenSpawn.Spawn(modExtension.projectileWhenSuccess, targets[0].Cell, pawn.Map);
                    projectile.Launch(pawn, targets[0].Cell.ToVector3(), randomOrigin, randomOrigin, ProjectileHitFlags.IntendedTarget);
                }
