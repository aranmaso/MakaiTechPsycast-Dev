...
namespace MakaiTechPsycast.CorruptedProphet
{
	...
	public class MakaiPsy_CP_CompPillarBeam : ThingComp
	{
		...
		private static readonly Material BeamMat = MaterialPool.MatFrom("Things/Mote/PowerbeamTexture", ShaderDatabase.Cutout, MapMaterialRenderQueues.OrbitalBeam);
		...
		public MakaiPsyCompProperties_PillarBeam Props => (MakaiPsyCompProperties_PillarBeam)props;
		...
		public override void PostDraw()
		{
			...
			if (TicksLeft > 0)
			{
				...
				float num = ((float)parent.Map.Size.z - drawPos.z) * 1.1f;
				...
				float num2 = Mathf.Min((float)TicksPassed / 1f, 1f);
				...
				Vector3 main = drawPos * num;
				pos1.z += Props.offsetZ;
				pos1.y = AltitudeLayer.MetaOverlays.AltitudeFor();
				Matrix4x4 matrix = default(Matrix4x4);
				matrix.SetTRS(vector2 + vector * BeamEndHeight * 0.5f + vector3, Quaternion.Euler(0f, 0f, 0f), new Vector3((Props.width * 3f) + shaking * 2.5f, 1f, num));
