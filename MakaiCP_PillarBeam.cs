...
{
	public class MakaiCP_PillarBeam : MakaiPsy_CP_PillarStrike
	{
		...
		{
			IntVec3 c = (from x in GenRadial.RadialCellsAround(base.Position, 5f, useCenter: true)
						 where x.InBounds(base.Map)
						 select x).RandomElementByWeight((IntVec3 x) => 1f - Mathf.Min(x.DistanceTo(base.Position) / 5f, 1f) + 0.05f);
			tmpThings.Clear();
			...
			for (int i = 0; i < tmpThings.Count; i++)
			{
				...
				tmpThings[i].TakeDamage(new DamageInfo(MakaiTechPsy_DefOf.CorruptedProphet_CorruptedSky, 5, 1f, -1f, instigator, null, weaponDef)).AssociateWithLog(battleLogEntry_DamageTaken);
			}
			tmpThings.Clear();
		}
	}
